...
Or paste full data from extension including song and hashtags..."
    ...
    function analyzeHashtags() {
        ...
        soundsWithData.forEach(([url, data]) => {
            ...
            // Also handle hashtags stored as object with counts
            Object.entries(data.hashtags || {}).forEach(([hashtag, count]) => {
                if (typeof count === 'number') {
                    hashtagFrequency[hashtag] = (hashtagFrequency[hashtag] || 0) + count;
                    soundHashtags.add(hashtag);
                }
            });
